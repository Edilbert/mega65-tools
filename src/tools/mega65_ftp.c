...
int main(int argc,char **argv)
{
  ...
  if (bitstream) {
    ...
    system(cmd);
#ifdef WINDOWS
    // TODO: check how well this compiles via the cross-compiler
...
{
  if (0)  fprintf(stderr,"Decoding raw byte $%02x, data_byte_count=$%04x\n",
		  v,data_byte_count);
  if (queue_read_len<1024*1024) queue_read_data[queue_read_len++]=v;
  if (data_byte_count) data_byte_count--;
}
...
void job_process_results(void)
{
  ...
  while (1) {
	...
	if (fn==2) {
	  ...
	  q_rle_count=0; q_raw_count=0; q_rle_enable=1;
...
int read_sector(const unsigned int sector_number,unsigned char *buffer,int useCache,int readAhead)
{
  ...
  do {
    ...
    if (readAhead>16) batch_read_size=readAhead;
...
unsigned int get_next_cluster(int cluster)
{
  ...
  do {
    ...
    if (read_sector(partition_start+fat1_sector+cluster_sector_number,buf,CACHE_YES,0)) break;
